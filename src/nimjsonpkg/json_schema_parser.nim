...
proc toObjectDefinitions(schema: JsonSchema, objectName: string, isPublic: bool, forceBackquote: bool): seq[ObjectDefinition] =
  ...
  for k, prop in schema.properties:
    ...
    let fDef = newFieldDefinition(typ.typeStr, typ, isPublic, forceBackquote, false, isOption)
    objDef.addFieldDefinition(fDef)
  result.add(objDef)

proc parse(s: string, objectName: string, isPublic: bool, forceBackquote: bool) =
